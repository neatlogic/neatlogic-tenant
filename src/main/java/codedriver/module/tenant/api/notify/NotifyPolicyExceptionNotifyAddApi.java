...
public class NotifyPolicyExceptionNotifyAddApi extends PrivateApiComponentBase {
    ...
    @Input({@Param(name = "policyId", type = ApiParamType.LONG, isRequired = true, desc = "策略id"), @Param(
        name = "userUuidList", type = ApiParamType.JSONARRAY, isRequired = true, desc = "用户uuid列表，格式[\"user#uuid\"]")})
    @Output({@Param(name = "userList", explode = UserVo[].class, desc = "用户信息列表")})
