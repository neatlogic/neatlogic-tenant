...
public class ToggleActivePortalApi extends PrivateApiComponentBase {
    ...
    public String getToken() {
        return "portal/toggleactive";
    }

    @Override
    public String getName() {
        return "nmtcap.toggleactiveportalapi.getname";
    }

    @Input({
            @Param(name = "id", type = ApiParamType.LONG, desc = "id", isRequired = true),
            @Param(name = "isActive", type = ApiParamType.INTEGER, rule = "0,1", isRequired = true, desc = "common.isactive")})
    @Description(desc = "nmtcap.toggleactiveportalapi.getname")
    @Override
    public Object myDoService(JSONObject paramObj) throws Exception {
        ...
        if (!Objects.equals(isActive, portalVo.getIsActive())) {
            portalVo.setIsActive(isActive);
            portalMapper.updatePortalActive(portalVo);
        }
        return null;
    }
}
