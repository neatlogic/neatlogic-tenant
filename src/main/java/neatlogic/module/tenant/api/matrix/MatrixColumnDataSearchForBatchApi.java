...
public class MatrixColumnDataSearchForBatchApi extends PrivateApiComponentBase {
    ...
    @Input({
            @Param(name = "searchParamList", type = ApiParamType.JSONARRAY, isRequired = true, minSize = 1, desc = "查询参数列表")
    })
    @Output({
            @Param(name = "list", type = ApiParamType.JSONARRAY, desc = "查询结果列表")
    })
    @Description(desc = "批量矩阵属性数据查询-下拉接口")
    @Override
